...
| **完全非纯末节区域<br>（Totally NSSA区域）** | 在NSSA基础上，不再接收Type-3 LSA，保留NSSA其他特性 | Type-1/2 + Type-7 | Type-3、Type-4/5 | 需手动配置或由ABR下发 |
  ...
  <details><summary><font size="7"> 2）OSPF网络类型 </summary>
...
| **广播型网络**<br>（Broadcast） | **以太网（Ethernet）**|**局域网（LAN）** | **需要** | 组播发送<br>（224.0.0.5/6） | 多路访问环境中，通过选举DR/BDR来减少邻接关系数量和LSA泛洪 |
| **非广播多路访问网络**<br>（NBMA） | **[帧中继](https://zh.wikipedia.org/wiki/%E5%B8%A7%E4%B8%AD%E7%BB%A7)、ATM、X.25** | **需要** | 单播发送 | 物理上多路访问，但不支持广播/组播。需手动配置邻居，选举DR/BDR。典型场景：传统广域网。 |
...
| **Type-7** | NSSA外部LSA | **ASBR（在NSSA内）** | NSSA区域专用，描述引入的外部路由。它仅在NSSA区域内泛洪，到达ABR后会被转换为Type-5 LSA，再传播到其他区域。 | （文字说明） |
  ...
  <details><summary><font size="7"> 5）OSPF报文 </summary>

> OSPF协议直接运行在IP层之上，端口89，使用组播地址*224.0.0.5*（所有SPF路由器）和*224.0.0.6*（DR/BDR）

<span id="OSPF_type"> OSPF的5种报文类型 </span>
| 报文类型 | 名称 | 作用 | 关键内容/说明 |
| :--- | :--- | :--- | :--- |
| **Type 1** | **Hello报文** | 发现邻居、建立和维护邻接关系、选举指定路由器DR/备份指定路由器BDR | 包含Router ID、Hello/Dead间隔、邻居列表、DR/BDR地址等。周期性组播发送 |
| **Type 2** | **数据库描述报文**<br>（Database Description，DD） | 在主/从关系建立后，描述本地LSDB的摘要信息（LSA头部列表），用于同步数据库 | 相当于LSDB的目录，不包含LSA的完整内容。通过隐式确认机制保证可靠传输 |
| **Type 3** | **链路状态请求报文**<br>（Link State Request，LSR） | 向邻居请求自己缺失或更新的LSA的完整信息 | 包含所需LSA的类型、链路状态ID、通告路由器 |
| **Type 4** | **链路状态更新报文**<br>（Link State Update，LSU） | 用于发送完整的LSA（可以包含多个LSA），作为对LSR的响应，或主动洪泛更新的LSA | 核心的LSA承载报文。收到LSU后，接收方需发送LSAck确认 |
| **Type 5** | **链路状态确认报文**<br>（Link State Acknowledgment，LSAck） | 对接收到的LSA进行确认，确保洪泛的可靠性 | 包含收到的LSA头部列表 |
...
|字段|解释|
|---|---|
|版本|代表OSPF版本号，v2用于IPv4，v3用于IPv6|
|类型|当前[报文类型](#OSPF_type)|
|分组长度|整个OSPF报文的长度|
|路由器ID|利用[路由器环路接口](https://zh.wikipedia.org/wiki/%E5%9B%9E%E7%8E%AF)的IP地址作为路由器标识，如果没有则选择最大的接口IP地址作为路由器标识|
|区域ID|在多区域网络中，每一个区域指定一个区域ID|
|认证类型|0-无认证，1-简单口令认证，2-加密认证（如MD5）|
  </details>
  <details><summary><font size="7"> 6）OSPFv3 </summary>

> OSPFv3（OSPF Version 3）由 [RFC 2740](https://datatracker.ietf.org/doc/html/rfc2740) 定义，是 OSPF 支持 IPv6 的版本。<br>与 OSPFv2 相比，其主要差异包括：

- 1.基于链路运行
  - 不再基于 IP 子网，同一链路上不同子网的路由器可直接建立邻居
- 2.地址协议分离
  - Router ID 仍为 32 位，与 IPv6 地址无关Router-LSA 和 Network-LSA 不再携带 IPv6 前缀
- 3.新增 LSA 类型
  - Link-LSA (Type 8)：通告链路本地地址和链路前缀
  - Intra-Area-Prefix-LSA (Type 9)：将 IPv6 前缀关联到 Router/Network-LSA
- 4.使用链路本地地址
  - 协议报文以 FE80::/10 作为源地址和下一跳，无需配置全球地址即可运行
- 5.依赖 IPsec 认证
  - 移除了 OSPFv2 的认证字段，完全依赖 IPv6 的 IPsec 保证完整性、真实性
- 6.支持多实例
  - 报文头新增 Instance ID，允许同一链路上运行多个独立 OSPFv3 实例
  </details>
