...
|<span id="ipconfig_renew"> `/renew `[*Adapter*] </span>|更新指定网卡的 DHCP 配置|
...
|<span id="tracert_w"> `-w `*Timeout* </span>|说明了等待<abbr title="Type 0： Echo Reply (回声响应)">ICMP type 0</abbr>和<abbr title="Type 11： (Time Exceeded (回声超时)">ICMP type 11</abbr>的时间|
...
|<span id="nslookup_host">`host`</span>|正向解析时输入域名，反向解析时直接输入 IP 地址|
|<span id="nslookup_server">`server`</span>|指定用于查询的 DNS 服务器（IP 地址），不指定则使用系统默认 DNS|
